...
if (type == "title"){
  corp = data$results__books__title}

if (type == "desc"){
  corp = data$results__books__description}
...
for (i in 1:opt$numcluster) {
  ...
  prep <- estimateEffect(1:(opt$numcluster)~(results__bestsellers_date), descfit, meta=out1$meta,
                         uncertainty="Global")
